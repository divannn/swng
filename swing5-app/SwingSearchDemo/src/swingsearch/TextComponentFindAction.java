 ...
 *
 */
public class TextComponentFindAction extends FindAction
	...
	protected boolean changed(JComponent comp, String str, Position.Bias bias) {
		...
		int offset = bias == Position.Bias.Forward ? textComp.getCaretPosition() : textComp
				.getCaret().getMark() - 1;
		...
		if (index != -1) {
			...
			offset = bias == null || bias == Position.Bias.Forward ? 0 : textComp.getDocument()
					.getLength();
	...
	protected int getNextMatch(JTextComponent textComp, String str, int startingOffset,
			Position.Bias bias) {
		...
		return bias == null || bias == Position.Bias.Forward ? text.indexOf(str, startingOffset)
